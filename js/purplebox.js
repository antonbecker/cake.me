...
const msg = require('./utils/msg')('purplebox'),
...
const Ghostery = function (win, doc) {
	...
	let ghostery, box, count, pbIcons, breakingIcon, slowIcon, nonSecureIcon, nonSecureSlowIcon, title, minimizeContainer, minimizeIcon, close, background, list;
	...
	const doSizeChange = function () {
		const windowHeight = doc.documentElement.clientHeight * 0.85 - 35;
	...
	const setBoxHeights = function () {
		var height, windowHeight, listHeight, digits;
	...
	const createTrackerDiv = function (app) {
		...
		return { tracker: liDiv, hasIcon: hasIcon };
	};
	...
	const handleNewTrackers = function (apps) {
		...
		for (let n = 0; n < apps.length; n++) {
			let appName = apps[n].name.toLowerCase();

			let appCategory = apps[n].cat.toLowerCase();
			if (!trackers.length) {
				let trackerObj = createTrackerDiv(apps[n]);
				appendChild(list, trackerObj.tracker);
			} else {
				let placed = false;
				for (let m = 0; m < trackers.length; m++) {
					let trackerName = trackers[m].textContent.toLowerCase();
					let trackerCategory = trackers[m].getAttribute("category");
...
module.exports = function (origin) {
	const onMessage = chrome.runtime.onMessage,
	      DEBUG = chrome.runtime.getManifest().debug || false,
	      LOG = chrome.runtime.getManifest().log || false,
